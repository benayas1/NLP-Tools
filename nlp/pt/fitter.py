...
class AverageMeter(object):
    ...
    Attributes:
        val (float): Stores the average loss of the last batch
        avg (float): Average loss
        sum (float): Sum of all losses
    ...
    def update(self, val, n=1):
        ...
        Args:
            val (float): Loss on each training step
...
class TorchFitterBase:
    ...
    def __init__(self,
        ...
        Args:
            ...
            multilabel (bool, optional): Parameter to define whether labels are MultiLabel or SL.
    ...
    def reduce_loss(self,
        ...
        """
        Reduces the loss to a scalar, if needed. This also applies weights if provided.

        Args:
            loss (torch.Tensor): Calculated loss
            weights (torch.Tensor): Weights by sample
        Returns:
    ...
    def fit(self,
        ...
        Args:
            ...
            save_checkpoint (str, optional): Whether to save the checkpoint when training. Defaults to None.
            save_best_checkpoint (str, optional): Whether to save the best checkpoint when training. Defaults to None.
        ...
        for e in range(n_epochs):
            ...
            if save_checkpoint is not None:
                self.save(save_checkpoint, False)
            ...
            if (
                ...
                self.model.eval()
                if save_best_checkpoint is not None:
                    self.save(save_best_checkpoint)
    ...
    def train_one_epoch(self, train_loader, verbose_steps=0):
        ...
        Args:
            train_loader (torch.data.utils.DataLoader): DataLoaders containing the training dataset
            verbose_steps (int, optional): Number of step to print every training summary. Defaults to 0.

        Returns:
            AverageMeter: Object with this epochs's average loss
        """
    ...
    def validation(self, val_loader, metric=None, metric_kwargs={}, verbose_steps=0):
        ...
        Returns:
            (AverageMeter, float): Object with this epochs's average loss and Calculated metric if a metric is provided, else None
        """
    ...
    def predict(self, test_loader, verbose_steps=0):
        ...
        Returns:
            np.array: Predicted values by the model
        """
